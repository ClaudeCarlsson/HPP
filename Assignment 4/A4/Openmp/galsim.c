...
double **get_dist(double **restrict distances, particle_t *restrict particles, const int N, int n_threads)
{
#pragma omp parallel for schedule(dynamic, n_threads * 10)
    for (int i = 0; i < (N - 1); i++)
        for (int j = i + 1; j < N; j++)
        {
...
int main(int argc, char *argv[])
{
    ...
    const int nsteps = atoi(argv[3]);        // How many steps to simulate
    ...
    for (int i = 0; i < N; i++)
    {
        if (!fread(&particles[i], sizeof(particle_t), 1, file))
        {
            printf("Error loading particles.");
        }
    }
    fclose(file);
    ...
    if (graphics == 0)
    {
        ...
        {
            // get all distances
            distances = get_dist(distances, particles, N, n_threads);
